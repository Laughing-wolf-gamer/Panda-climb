...
public class MasterController : MonoBehaviour {
	...
	private CommonSaveSystemService commonSaveSystemService;
    private void Awake(){
        current = this;
		commonSaveSystemService = new CommonSaveSystemService(new PlayerPrefsUsernameStore(saveController._config), new HttpScoreApiClient(saveController._config));
    }
    private void Start(){
        Time.timeScale = 1f;
    ...
    private IEnumerator GamePlayingRoutine(){
		...
		_= saveController.SubmitScoreAsync((int)longestDistance);
		commonSaveSystemService.SubmitScoreAsync((int)longestDistance);
